...
export class Child extends FamilyMemberBase {
  static override checkIfMember = (memberString: string) => {
    return /^[A-Z]([a-z]|[^\x00-\x7F])+\s+[A-Z]([a-z]|[^\x00-\x7F])+$/.test(
      memberString.trim()
    );
  };
  ...
  static override extractNameLastName = (
    ...
    const [firstName, lastName] = personString.trim().split(' ');
...
Family.FamilyMembersTypes.push(Child);
